...
void cp_init_blk (struct cache_blk_t *blk) {
blk->bdi_encode = (byte_t) -1;
blk->bdi_mask = (sword_t) -1;
}

void cp_cache_reg_stats ( struct cache_t *cp, struct stat_sdb_t *sdb )	
{
...
void cp_update_blk (struct cache_blk_t *blk, byte_t bdi_encode, qword_t bdi_mask) {
		...
		struct stat_sdb_t *sdb)	/* stats database */
{
  ...
  cp_cache_reg_stats(cp, sdb); //sdrea
             ...
             struct mem_t *mem) //sdrea
{
  ...
  cp_update_blk(repl, bdi_encode, bdi_mask); //sdrea
